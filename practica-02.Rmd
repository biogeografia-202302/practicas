...
scatterplot3d(ma$`especie 1`, ma$`especie 2`, ma$`especie 3`,
...
```{r, echo=F, inlude=F}
as.matrix(dist(ma, diag = T, upper = T)) %>%
  as.data.frame() %>% 
  rownames_to_column(var = ' ') %>% estilo_kable() %>% 
  column_spec(1, bold = TRUE)
```
